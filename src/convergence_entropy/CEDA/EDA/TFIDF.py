...
class TFIDF():
    ...
    def fit(self, df):
        ...
        # w = t.sum(axis=1)
        # tf = np.divide(t.T, w)
        # sum_t = (t > 0).astype(float).sum(axis=0) + 1e-9
        sum_t = (t).astype(float).sum(axis=0) + 1e-9

        # implements IDF for c-TF-IDF as described by M Grootendorst
        # idf = np.log(np.divide(len(dft), sum_t + 1)).reshape(-1, 1)

        # implements a corpus wide IDF . . . the idea being that we want
        #  to compare each document against anything other possible thing
        #  that can be said.
        idf = np.log(np.divide(sum_t.sum(), sum_t)).reshape(-1)
    ...
    def ctfidf(self, axes):
        tf = self.tf_[axes].sum(axis=0)
        tf = tf / tf.sum()
        return tf * self.idf
    ...
    #################################################################################
    # ToDo:
    #   (1) make each function require setting specific axes or sets of axes to run.
    #       (1.1) contemplate having each function be individualized, or require a
    #           c-tf-idf output to be passed to it as a parameter? That last option
    #           is probably going to be the smoothest experience/create the lowest
    #           lag.
    #################################################################################
    def pred_batch(self, sentences):
        val = self.count.transform(sentences).toarray()
        tf = self.tf_ / self.tf_.sum(axis=-1).reshape(-1, 1)
        res = val @ (tf * self.idf)
    ...
    def __topnw(self, ctfidf, n=20):
        words = self.count.get_feature_names_out()
        sorted = ctfidf.argsort()[::-1][:n]
        return list(zip(words[sorted], ctfidf[sorted]))

    def topnw(self, topic, n=20):
        axis = topic
        if not isinstance(axis, list):
            axis = [axis]
        return self.__topnw(self.ctfidf(axis), n=n)

    def get_topic_names(self):
        names = self.extract_top_n_words_per_topic(n=3)
        return {k: '_'.join([str(k)] + [str(j[0]) for j in v]) for k, v in names.items()}

    def plot_topic_word_frequencies(self, specific_topics, k_words: int=5, n_cols:int=3, height:int=250, width:int=250, round_to:int=9):
        topics = {str(topic): self.topnw(topic,n=k_words) for topic in specific_topics}
...
class TFIDF_v1():
    ...
    def c_tf_idf(self, df):
        ...
        #  to compare each document against anything other possible thing
        #  that can be said.
        idf = np.log(np.divide(sum_t.sum(), sum_t + 1e-9)).reshape(-1, 1)
